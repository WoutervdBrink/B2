...
class Lijst
{
    ...
    public function voegToe(Contact $contact)
    {
        ...
        if ($this->grootte == 0) {
        ...
        } else if ($this->eerste->contact->voornaam >= $contact->voornaam) {
            ...
            while (!empty($element->volgende) && $element->volgende->contact->voornaam < $contact->voornaam) {
            ...
            if (!empty($element->volgende)) {
                $nieuwElement->volgende = $element->volgende;
            }

            // Ten slotte voegen we het nieuwe element in als volgende element van het gevonden element.
